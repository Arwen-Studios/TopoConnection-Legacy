	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length)
		{
			...
			tipText.y += i * 14;
			tipText.setFormat(Paths.font(Std.string(Main.gameFont)), 16, FlxColor.WHITE, LEFT /*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		tab_group_song.add(player2DropDown); // eu tirei isso pq as skins da notas é  das opçoes lmao -Luis
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			...
			if (i[3] != null && note.noteType != null && note.noteType.length > 0)
			{
				...
				daText.setFormat(Paths.font(Std.string(Main.gameFont)), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if (endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Paths.font(Std.string(Main.gameFont)), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
				if (note.eventLength > 1)
					daText.yAdd += 8;
