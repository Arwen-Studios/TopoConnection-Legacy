...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		tipText.setFormat(Paths.font(Std.string(Main.gameFont)), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipText.cameras = [camOther];
		tipText.scrollFactor.set();
		add(tipText);

		offsetLoopText = new FlxText(10, 10, 0, '', 32);
		offsetLoopText.setFormat(Paths.font(Std.string(Main.gameFont)), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		offsetIdleText.setFormat(Paths.font(Std.string(Main.gameFont)), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		animText.setFormat(Paths.font(Std.string(Main.gameFont)), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				FlxG.sound.playMusic(Paths.music(Main.menuSong), 1);
				transitioning = true;
			}
