...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'white-space': // Chapter 1 - Week 1
				...
				bg.screenCenter();
				add(bg);
				bgBlack = new FlxSprite(0, 0).makeGraphic(Std.int(FlxG.width * 3), Std.int(FlxG.height * 3), FlxColor.BLACK);
		...
		if (ClientPrefs.laneOpacity > 0 && ClientPrefs.bgAlpha < 1)
		{
		...
		timeTxt.setFormat(Paths.font(Std.string(Main.gameFont)), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		timeBarOV = new AttachedSprite('timeBarOV');
		timeBarOV.x = timeTxt.x;
		timeBarOV.y = timeTxt.y + (timeTxt.height / 4);
		timeBarOV.scrollFactor.set();
		timeBarOV.alpha = 0;
		timeBarOV.visible = showTime;
		timeBarOV.xAdd = -4;
		timeBarOV.yAdd = -4;
		timeBarOV.screenCenter(X);
	...
	{ // STOLE FROM ANDROMEDA
		switch (cam.toLowerCase())
		{
			case 'camhud' | 'hud':
				...
				{
					newCamEffects.push(new ShaderFilter(i.shader)); // IT SHUTS HAXE UP IDK WHY BUT WHATEVER IDK WHY I CANT JUST ARRAY<SHADERFILTER>
				}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (ClientPrefs.getGameplaySetting("noHolds", false))
			allowScoring = false;
		var transformation:Array<Int> = [0, 1, 2, 3];

		if (ClientPrefs.getGameplaySetting("right", false))
		{
			transformation = [2, 0, 3, 1];
		}
		...
		if (ClientPrefs.getGameplaySetting("noJacks", false))
		{
			...
			for (note in unspawnNotes)
			{
				...
				{
					if (!note.mustPress)
						pastDadNotes[note.noteData] = note;
					else
						pastBFNotes[note.noteData] = note;
				}
			}
		}
		for (note in unspawnNotes)
		{ // because eradicating jacks caused issues w/ holds!!
			if (note.isSustainNote)
			{
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1)
				babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player, dad.noteSkinFile);
			else
				babyArrow = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player, boyfriend.noteSkinFile);
	...
	override public function update(elapsed:Float)
	{
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		if (songName != "bimbo"
			&& FlxG.keys.anyJustPressed(debugKeysChart)
			&& !endingSong
			&& !inCutscene
			&& !cpuControlled
			&& !practiceMode
		...
		else
		{
			...
			if (!paused)
			{
				...
				if (updateTime)
				{
					...
					if (ClientPrefs.timeBarType == 'Time Elapsed' || ClientPrefs.timeBarType == 'Time Elapsed & Name')
						songCalc = curTime;
					...
					if (ClientPrefs.timeBarType == 'Time Left & Name' || ClientPrefs.timeBarType == 'Time Elapsed & Name')
						timeTxt.text += ' | ${SONG.song}';
				}
			}
